...
def calculate_wellness_score(data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    The score is built from a weighted combination of key wellness signals.
    This ensures changes in inputs (e.g., screen time, sleep, notifications, steps)
    produce proportional changes in the final score.
    ...
    heart_rate = _parse_float(data.get("heart_rate") or 0)
    steps = _parse_float(data.get("steps") or data.get("steps_today") or 0)

    # Normalize to 0..1 with realistic bounds
    # Lower screen time is better (0 min = best, 360 min = worst)
    screen_norm = max(0.0, min(1.0, 1.0 - screen_time / 360.0))
    # Optimal sleep roughly 8h; values between 4-9 impact the score
    sleep_norm = max(0.0, min(1.0, (sleep_hours - 4.0) / 5.0))
    # Fewer notifications is better; clamp at 200
    notifications_norm = max(0.0, min(1.0, 1.0 - notifications / 200.0))
    # Heart rate: 70 bpm is ideal, penalize deviations (set range roughly 40-100)
    hr_diff = abs(heart_rate - 70.0)
    heart_rate_norm = max(0.0, min(1.0, 1.0 - hr_diff / 30.0))
    # More steps is better; cap at 10k
    steps_norm = max(0.0, min(1.0, steps / 10000.0))

    # Weighted score components
    screen_score = screen_norm * 25.0
    sleep_score = sleep_norm * 25.0
    notifications_score = notifications_norm * 15.0
    heart_rate_score = heart_rate_norm * 10.0
    steps_score = steps_norm * 25.0

    total = round(
        max(0.0, min(100.0, screen_score + sleep_score + notifications_score + heart_rate_score + steps_score)),
        1,
    )
    ...
    return {
        ...
        "status": status,
        "breakdown": {
            "screen_time": round(screen_score, 1),
            "sleep_hours": round(sleep_score, 1),
            "notifications": round(notifications_score, 1),
